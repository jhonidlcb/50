  ...
  try {
    ...
    const payload = [{
      ...
      items: documento.items.map(item => {
        // Construct item WITHOUT any 'iva' key
        return {
...
export async function construirDocumentoFacturaSend(company: any, client: any, stage: any, project: any, exchangeRate: number, numero: number): Promise<FacturaSendDocumento> {
  const monto = Math.round(parseFloat(stage.amount) * exchangeRate);
  const precioSinIva = Math.round(monto / 1.1);
  ...
  return {
    ...
    condicion: {
      tipo: 1,
      entregas: [{ tipo: 9, monto, moneda: 'PYG' }]
    },
    items: [{
